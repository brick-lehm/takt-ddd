...
端末を実行に接続したままにする場合は、workflow を直接実行します。

```bash
takt --workflow my-workflow --task "依頼された変更を実装する"
```

workflow の step または transition に `requires_user_input: true` が設定されている場合、TAKT はその判断を求める内容を同じ端末に表示して入力待ちになります。入力内容は同じ workflow run に渡され、判断地点から処理を継続します。

失敗または中断した直接実行を `takt resume` で再開する場合も同じです。対象は端末前面で動く直接実行だけです。キュー実行の `takt run` と stdio MCP 実行は、並列またはリモート実行が端末入力待ちで停止しないよう、非対話のままです。
